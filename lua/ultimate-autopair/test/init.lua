...
function M.check_not_allowed_files_and_strings(paths)
...
function M.check_other(_)
    ...
    if not pcall(require,'nvim-treesitter') then
        M.fn.warning('nvim-treesitter not installed: most of treesitter specific behavior will not work')
    end
end
function M.start()
    ...
    if _G.UA_DEV then
        M.check_not_allowed_files_and_strings(paths)
    end
    M.check_other(paths)
