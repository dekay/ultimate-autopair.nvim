...
if _G.UA_IN_TEST then pcall(require,'ua-test') end
local M={}
...
function M.run_tests(conf,tests,has_not_ok)
    ...
    local err_,info_=pcall(ua.setup,conf)
    if not err_ then
        M.fn.error(('DEBUG: the config (or a PREVIOUS config) caused an error, conf: %s, error: %s'):format(vim.inspect(conf),info_))
    end
    for _,testopt in ipairs(tests) do
        local category=testopt._category
        testopt._category=nil
...
function M.run_test(testopt)
    ...
    local action=vim.api.nvim_eval(M.get_rhs(key))
